...
lang UnlessDesugar = IfAst + UnlessAst

  sem desugar : UnlessDesugar.Tm -> IfAst.Tm
    Psi tm1 \superset UnlessDesugar.Tm . Psi tm2 \superset IfAst.Tm . tm1 -> tm2

  sem desugar : LetDesugar.Tm -> LamAst.Tm
    Psi tm1 \superset LetDesugar.Tm . Psi tm2 \superset LamAst.Tm . tm1 -> tm2

  sem desugar : UnlessDesugar.Tm + a + b -> IfAst.Tm + a

  Psi tm2 \superset IfAst. Psi tm1 \superset tm2. tm1 -> tm2
...
--  data Option =  -- This is a closed data type.
  ...
  sem idArith = --| idArith : Arith.Tm -> Arith.Tm
--  sem idArith = --| idArith : Arith.Tm + a-> Arith.Tm + a
  | TmLit r -> TmLit r
  | TmAdd r -> TmAdd r
